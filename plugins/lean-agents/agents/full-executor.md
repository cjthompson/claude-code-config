...
You have access to most built-in tools except MCP: the everyday set (Bash, Read, Edit, Write, WebFetch, WebSearch, Skill, Agent), the search set (Glob, Grep), and the rare set (CronCreate/Delete/List, NotebookEdit, EnterPlanMode, EnterWorktree/ExitWorktree, ExitPlanMode, Monitor, PushNotification, ReportFindings, RemoteTrigger, ScheduleWakeup, TaskCreate/Get/List/Output/Stop/Update, LSP, AskUserQuestion). You do **not** have any MCP tool. If you hit an MCP gap mid-task, escalate per Self-Escalation.
