...
- **X needs a rare built-in tool** (Cron*, Worktree, Monitor, Task*, NotebookEdit, LSP, AskUserQuestion) — spawn `full-executor`. Note that LSP means *semantic* code navigation (definitions, references, symbol types); plain text or filename search does not qualify.
