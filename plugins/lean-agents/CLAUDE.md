...
- **Search is not a reason to move up the ladder.** A sub-task that needs text search or filename/path matching stays on `standard-executor` (or `lean-executor`) — both have Bash, which covers `Grep` with `rg`/`grep` and `Glob` with `find` without adding a tool to the roster. Of the *search* capabilities, only *semantic* navigation (`LSP` — go-to-definition, find-references) requires `main` or `full-executor`. This bullet narrows nothing else — every other trigger in the next bullet still applies.
  - When dispatching a Bash search, tell the worker to bound the output: scope it to a path, exclude vendored trees (`node_modules/`, `.git/`), and cap results (`rg -l`, `rg -c`, `| head -n 100`). An unbounded `grep -rn` in a repo that vendors `node_modules/` can cost far more context than the `full-executor` spawn it was meant to avoid. Workers need `Bash(rg:*)` / `Bash(grep:*)` / `Bash(find:*)` permitted — `lean-executor` has no `Agent` tool and cannot escalate if a search is denied.
- If a `standard-executor` or `main` reports a tool gap, escalate to `full-executor` (rare tools: Cron*, NotebookEdit, Worktree, Monitor, PushNotification, ReportFindings, RemoteTrigger, ScheduleWakeup, Task family, LSP, AskUserQuestion) or `general-purpose` (MCP). `standard-executor` can self-escalate on its own — let it.
- `EnterPlanMode`/`ExitPlanMode` are not escalatable to any sub-agent, full stop — the harness blocks both tools inside subagent context regardless of the target profile's `tools:` frontmatter (confirmed by a runtime error: "ExitPlanMode is not available inside subagents"). `full-executor` still lists them for the (untested) case where it's run as the top-level session agent itself, but no profile should ever spawn another agent to handle plan mode. `main`, as the profile meant to run at the top level, instead asks the user directly to exit plan mode and confirm before proceeding — see `agents/main.md`'s Plan Mode Handoff section.
