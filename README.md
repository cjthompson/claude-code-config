...
A curses TUI to manage per-repo Claude Code opt-ins across four tabs: Plugins (with their skills and agents), MCP servers (discovered from `.mcp.json` files and `~/.claude.json`), individual Skills (personal, project, and active-plugin, each with its own `on`/`name-only`/`user-invocable-only`/`off` state), and Trust (Claude Code's per-project trust flag in `~/.claude.json`, also scriptable via `--trust`, `--untrust`, and `--trust-status`). Shows the effective state and where it comes from (user / project / local settings) and lets you toggle a local override. Located in `packages/claude-optin/`. Install via the TUI installer or `npm run install-package claude-optin` (installs to `~/.local/bin/`), then run `claude-optin` from inside a repo (assuming `~/.local/bin` is on your `PATH`).
