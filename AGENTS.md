...
This repository contains configurable elements for Codex, including skills, statusline customizations, and package configurations. When making changes to items contained in this repo:
...
- **AskUserQuestion: Run `install-packages`?** — After changes are committed and merged, ask the user if they want to run `npm run install-packages` to reinstall/apply updates to your Codex configuration.

- **`~/.codex` is off-limits** — Never read or write files directly under `~/.codex/`. Always work in this repository (`plugins/`, `packages/`, etc.). The installed copies under `~/.codex/` are symlinked and will reflect changes automatically.
