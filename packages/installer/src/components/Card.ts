...
interface CardProps {
    ...
    description: string;
    enabled: boolean;
    items: Array<{ name: string; enabled: boolean; alreadyInstalled: boolean; needsUpgrade?: boolean; isCurrent?: boolean; markedForRemoval?: boolean }>;
    focused: boolean;
    focusedItem: number;
}
...
export function Card({ label, description, enabled, items, focused, focusedItem }: CardProps) {
    ...
    const hasUpgrades = items.some((i) => i.needsUpgrade);
    const status = hasRemovals ? "REMOVE" : hasUpgrades ? "UPGRADE" : enabled ? "ON" : "OFF";
    const statusColor = hasRemovals ? "red" : hasUpgrades ? "yellow" : enabled ? "green" : "gray";
